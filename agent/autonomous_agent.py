...
import os, time, subprocess, requests, random
...
class AutonomousAgent:
    ...
    def get_tier(self, rep=None):
        ...
        else: return "NO_SOUL", 100

    def ensure_soul(self):
        print("[SOUL] Checking identity...")
        if self.has_soul():
            self.token_id = self.get_token_id()
            tier, _ = self.get_tier()
            print(f"[SOUL] Found! Token #{self.token_id}, Rep: {self.get_reputation()} ({tier})")
            return True
